...
| ![](../Images/note.png) | 注意:你可以在下面的 Git 资源库中找到完整的源代码:[https://bitbucket . org/syncfusiontech/entity-framework-code-first-简洁地/overview](https://bitbucket.org/syncfusiontech/entity-framework-code-first-succinctly/overview) 。 |
...
| ![](../Images/tip.png) | 提示:请注意，实体集合的 setters 和 getters 都是公共的。 |

| ![](../Images/note.png) | 注意:请随意将您自己的方法、业务或其他方法添加到上下文类中。 |
...
| ![](../Images/note.png) | 注意:仅仅通过观察一个端点，我们不能立即判断它的类型(一对一或多对一)，我们需要观察两个端点。 |
...
| ![](../Images/note.png) | 注意:引用和集合统称为导航属性，而不是标量属性。 |
...
| ![](../Images/tip.png) | 提示:当此属性应用于字符串属性时，它不仅可以防止属性为空，还可以防止接受空字符串。 |

| ![](../Images/tip.png) | 提示:对于值类型，应该适当选择实际的属性类型。如果列不可为空，则不应选择可为空的属性类型，如 Int32？。 |
...
| ![](../Images/note.png) | 注意:配置关系时，您只需要配置一个端点。 |
...
| ![](../Images/note.png) | 注意:对于自动生成代码的情况，这是一个很好的解决方法，前提是所有类都是部分生成的。 |
...
| ![](../Images/note.png) | 注意:您可以在 OnModelCreating 方法和配置类中自由混合映射，但是您应该遵循一致的方法。 |
...
| ![](../Images/tip.png) | 提示:虽然其他数据库引擎中也存在类似的概念，但是实体框架只能在 SQL Server 中使用 IDENTITY。 |
...
| ![](../Images/note.png) | 注意:使用非整数标识符属性时，默认情况下数据库不会生成这些属性，因此您可以安全地跳过 DatabaseGeneratedAttribute。 |

| ![](../Images/note.png) | 注意:将 Guids 用于主键还有一个好处，就是可以将不同数据库中的记录合并到同一个表中；记录永远不会有冲突的关键字。 |
...
| ![](../Images/note.png) | 注意:实体框架的下一个版本 6.0 将支持自定义约定。 |
...
| ![](../Images/tip.png) | 提示:遗产丢失是因为英孚没有办法知道我们想要如何拥有它。 |
...
| ![](../Images/tip.png) | 提示:电动工具扩展还在测试阶段，所以你可以预期一些 bug 会存在。 |