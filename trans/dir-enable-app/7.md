...
| ![](../Images/note.png) | 注意:“权限标识”值必须是通过使用 GUID 生成工具或以编程方式创建的新生成的 GUID。它表示由网络应用编程接口公开的权限的唯一标识符。一旦您的客户端被适当地配置为请求访问您的网络应用编程接口并调用网络应用编程接口，它将呈现一个 OAuth 2.0 JWT 令牌，其范围声明被设置为上面的“声明值”，在本例中为“用户模拟”您可以在以后根据需要公开其他权限范围。 |
