...
| ![](../Images/note.png) | 注意:如前所述，原始 x86 寄存器的高字节形式不能与新 x64 寄存器的低字节形式一起使用。类似“MOV AH，R8B”的东西不会编译，因为它使用高字节形式 AH 和新的字节形式 R8B。高字节格式包含在 x64 中只是为了向后兼容。中央处理器不知道在一条指令中实现“MOV AH，R8B”的机器代码。 |
