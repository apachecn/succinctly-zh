...
| ![](img/note.png) | 注意:这种对堆栈的浪费可能是因为 C++编译器更容易编程。这种编程水平的许多东西几乎没有清晰的文档或解释。微软 C 调用约定使用阴影空间的确切原因尚不清楚。 |
...
| ![](img/note.png) | 注意:当子程序返回时，通过堆栈传递的参数实际上不会从内存中移除。堆栈指针简单地递增，使得新推送的参数将覆盖旧值。 |
