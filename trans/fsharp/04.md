...
| ![](../Images/note.png) | 注意:变量类型或类型参数化的概念与*泛型*的概念密切相关，后者是在 CLR 2.0 版本中引入的，现在已经成为 CLI 2.0 版本的 ECMA 规范的一部分。当 F#以启用了泛型的 CLI 为目标时，它会通过在找到未确定类型的任何地方使用它们来充分利用它们。F#的创建者 Don Syme 在。NET CLR 在他开始研究 F#之前。有人可能会想推断他这么做是为了创造 F#！ |
