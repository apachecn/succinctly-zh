...
从Stargate获取数据的另一种方法是创建一个行扫描器，它类似于服务器端的游标，您可以使用它来读取多行，并且扫描器可以选择有一个过滤器来限制返回的单元格数量。

用Stargate扫描行有两个部分。首先，创建运行在服务器上的扫描仪。星际之门给你一个扫描仪的参考，你可以用它来获取行。在。NET 客户端，您可以使用 ScannerOptions 对象来指定扫描的开始和结束行。
