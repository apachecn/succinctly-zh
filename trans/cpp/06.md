...
但是，您可能有类似数据数组的东西。您可能还希望该类的任何副本创建当时存在的数据的副本。在这种情况下，带有自定义复制构造函数的唯一指针可能是正确的选择。
...
除了 C++异常，没有堆栈跟踪(不包括调试器提供的堆栈跟踪功能)。因为捕获异常的 try-block 范围内的自动持续时间对象会在激活适当的 catch 处理程序(如果有)之前自动销毁，所以您没有时间检查可能导致异常的数据。您最初需要处理的只是来自`what`成员函数的消息。
