...
当我们的应用程序加载时，它会立即对初始用户数据发出 AJAX 请求。您的`/get-intial-data`服务器端脚本应该返回与本章[加载数据](#heading_id_68)部分的示例 JSON 输出相同的内容。一旦数据被加载，我们通过`ko.mapping.fromJS()`创建一个视图模型。这将采用脚本生成的本机 JavaScript 对象，并将每个属性转换为可观测的。除了`saveUserData()`和`loadUserData()`方法，这个动态生成的视图模型具有与`PersonViewModel`完全相同的功能。
