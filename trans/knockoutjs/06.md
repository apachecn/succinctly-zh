...
表单元素是通过网页与用户交互的常规方式。在 Denuke . js 中使用表单与使用外观绑定非常相似。但是，因为用户可以编辑表单域，所以knockout. js 管理两个方向的更新*。这意味着交互绑定是*双向的*。它们可以通过编程设置，视图将相应更新，*或*它们可以通过视图设置并通过编程读取。*
...
请注意，我们需要为knockout提供相同的对象引用(`brats`)给`products`和`favoriteProducts`，以正确初始化选择。
